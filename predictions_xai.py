...
def get_pickles():
	...
	df_sample_true = df_sample_true.sort_values(by='% certeza', ascending=False).round(2).head(5)
	df_sample_false = df_sample_false.sort_values(by='% certeza', ascending=False).round(2).head(5)
	...
	df_sample_true = df_sample_true[['Nome', 'Sobrenome'] + [cols[-1:] + cols[:-1]]]

	df_sample_false['Nome'] = ['Pedro', 'Georgina', 'Natalia', 'Thiago', 'Denise']
	df_sample_false['Sobrenome'] = ['Neto', 'Lima', 'Diaz', 'Batista', 'Garcez']
	df_sample_false = df_sample_false[['Nome', 'Sobrenome'] + [cols[-1:] + cols[:-1]]]
